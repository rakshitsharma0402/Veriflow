...
- **Rule execution is manual, not automated.** The schema (`Data
  Quality Rule`) fully supports automatic evaluation against Merchant
  Records, but the rule-engine hook itself wasn't built in this pass —
  exceptions are currently created directly rather than generated by
  a background evaluation job.
...
- Automated rule-engine hook on Merchant Record save
